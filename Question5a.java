...
class Question5a { 
    ...
    public static Node insertRec(Node node, int val) {
		...
		if (val < node.val) {
			node.left = insertRec(node.left, val); //if val < root, add to left tree 
		}

		if (val > node.val) {
			node.right = insertRec(node.right, val); //if val > root, add to right tree 
		}
    ...
    public static void main(String[] args) { 
        Question5a newTree = new Question5a(); 

        Node newRoot = null; 
        
		int[] vals = getRandomArray(10); 
        ...
        for (int i = 0; i < vals.length; i++) {
			newRoot = insertRec(newRoot, vals[i]);
		}
